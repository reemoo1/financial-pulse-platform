...
import CollateralOverview from "@/components/bank/CollateralOverview";

export default async function CollateralDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <BankShell
      user={session}
      title={`الضمانات — ${request.data.applicantName}`}
    ...
    >
      <div className="space-y-6">
        ...
        {request.data.collateral ? (
          <CollateralOverview collateral={request.data.collateral} />
        ) : (
          <div className="bank-card p-10 text-center">
            <p className="text-sm text-slate-500">لا توجد حزمة ضمان لهذا الطلب بعد.</p>
          </div>
        )}
      </div>
    </BankShell>
