  ...
  return (
    <section className="space-y-5" id="collateral-workspace">
      ...
      <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
        ...
        <div className={`rounded-2xl border p-5 ${tangibleRequired ? "border-amber-200 bg-amber-50" : "border-emerald-200 bg-emerald-50"}`}><div className="flex items-center gap-2"><Landmark className="h-5 w-5" /><h3 className="font-bold">قرار الضمان لهذا الطلب</h3></div><p className="mt-3 text-sm font-bold">{tangibleRequired ? `تغطية مطلوبة ${pct(collateral.requiredCoverageRatio, true)}` : "تمويل دون ضمان عيني"}</p><p className="mt-2 text-xs leading-6">{tangibleRequired ? "يجب رفع قيمة مؤهلة كافية قبل الموافقة النهائية، ثم استكمال التوثيق القانوني قبل الصرف." : "يكفي متطلب التوثيق الخفيف الظاهر في الحزمة، مثل سند لأمر، قبل تفعيل الصرف."}</p></div>
      </div>
